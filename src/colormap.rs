//! Colormaps for displaying slices and the 3-D rendering.
//!
//! Each colormap is compiled to a 256-entry RGB lookup table so the per-pixel
//! display path is just an array index (CPU slice views) or a 256x1 texture
//! lookup (GPU volume rendering). Scientific maps come from `colorgrad`;
